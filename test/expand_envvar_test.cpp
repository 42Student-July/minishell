...
TEST(filter_null_literal, normal)
{
	std::vector<std::pair<t_tokentype, std::string>> input_vector = {
		{TOKEN_IDENT, "hoge"},
		{TOKEN_IDENT, "NULL"},
	...
	t_list *input = init_input_lists(input_vector);
	t_list *output = filter_null_literal_token(input);
	ft_lstclear(&input, delete_token);
	compare_tokens(output, expected);
	ft_lstclear(&output, delete_token);
}

TEST(filter_null_literal, top_null)
{
	std::vector<std::pair<t_tokentype, std::string>> input_vector = {
		{TOKEN_IDENT, "NULL"},
		{TOKEN_IDENT, "NULL"},
